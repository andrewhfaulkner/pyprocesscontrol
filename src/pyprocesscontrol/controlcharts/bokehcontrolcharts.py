...
def basic_control_chart(
    ...
    for column in col_names:
        ...
        d = Div(text = title_str + column, styles = {'text-align': 'center', 'font-size': '16pt'})
        ...
        p = figure(sizing_mode = "stretch_width", 
                   ...
                   min_border = 0, x_axis_location = None, 
                   #title = title_str + column,
                   tools = "xpan,xwheel_zoom")
        
        

        h = figure(sizing_mode = 'stretch_height',
                   width = 300,
                   height = 300
                   )

        r = figure(sizing_mode = "stretch_width", 
                   height = 150,
                   width = 300, 
                   x_axis_type = 'datetime',
                   x_range = p.x_range
        ...
        p.hspan(data[column].mean(), 
                line_width = 1, 
                line_color = 'red', 
                line_dash = 'dashed'
                )
        
        p.hspan(upper_control,
                line_width = 1,
                line_color = 'black',
                line_dash = 'dashed')
        
        p.hspan(lower_control,
                line_width = 1,
                line_color = 'black',
                line_dash = 'dashed')
...
def ooc_checker(data: pd.DataFrame| pd.Series, ax: figure, column: str = None) -> None:
    ...
    index_list = pd.concat([index_list, pd.Series(less_than_window[less_than_window <= -2].index)])
...
def histogram_builder(
    ...
    ax.quad(top = counts, bottom = 0, left = bins[:-1], right = bins[1:],
            fill_color = 'skyblue', line_color = 'white')
    
    pdf = st.norm.pdf(x_axis, loc= stats[col_names].loc["mean"], scale = stats[col_names].loc['std'])
